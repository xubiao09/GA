...
Fuel_1=0;
for i=1:N/1
    [tempv,tempt,~,tempc_opt] = GA(tempv0,{green{i}-t0},distance(i),flag);
...
for i=1:N/2
    ...
    [tempv,tempt,~,tempc_opt] = GA(tempv0,tempgreen,distance(2*i-1:2*i),flag);
    TempFuel=-2000*log(1-tempc_opt(end));
    v_2=[v_2;tempv(:)];
    t0=t0+tempt;
...
for i=1:N/5
    ...
    [tempv,tempt,~,tempc_opt] = GA(tempv0,tempgreen,distance(5*i-4:5*i),flag);
    TempFuel=-2000*log(1-tempc_opt(end));
    v_5=[v_5;tempv(:)];
    t0=t0+tempt;
...
for i=1:N/10
    ...
    [tempv,tempt,~,tempc_opt] = GA(tempv0,tempgreen,distance(10*i-9:10*i),flag);
    TempFuel=-2000*log(1-tempc_opt(end));
    v_10=[v_10;tempv(:)];
    t0=t0+tempt;
...
% ylabel('Fuel[g]');
% grid on;
[h11,h12]=PlotTrajct(green,v0,v_1,distance);
[h21,h22]=PlotTrajct(green,v0,v_2,distance);
[h31,h32]=PlotTrajct(green,v0,v_5,distance);
[h41,h42]=PlotTrajct(green,v0,v_10,distance);
figure(1);
